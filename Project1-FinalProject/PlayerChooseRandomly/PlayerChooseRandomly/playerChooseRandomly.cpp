...
const int SleepTime = 0;  //the sleep time of the program(unit is second)
...
void Shuffle()
{
	srand((int)time(0));                                          //use time as srand
	for (int exchange1 = 0; exchange1 < o * k + g; exchange1++)   //exchange each card in the deck randomly
	{
		int exchange2 = rand() % (o * k + g);                     //another card index
		Card Medium;                                              //exchange by the medium card
...
//7-Play cards(screen shot when check the end)
