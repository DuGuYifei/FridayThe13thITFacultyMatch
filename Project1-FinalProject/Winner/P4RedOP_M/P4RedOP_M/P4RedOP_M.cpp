...
void EstimateValue(int valuen, int valuek, int valueg, int valuegv, int valueo)  //estimate whether the value exceed the requirement
{
	...
	while (valueo < 1 || valueo > 60) //estimate o
	{
		std::cout << "1<=o<=60,input o again" << std::endl;
		std::cin >> o;
		valueo = o;
	}
}
...
BattleCard ChooseMaxColourCard(int activeplayer)                                 //choose biggest card and smallest quantity of pile
{
	BattleCard Medium;                                                           //store the max colour card
	Medium.BasicInfo = Players[activeplayer].Card_Hand[0];                       //initialize the card is the first card in hand
	...
	{
		if (strcmp(Medium.BasicInfo.CardColour, "green") != 0)                   //if the first card is not green
		{
			int Number_MediumColour = 0;                                             //count how many card of this colour in grave yard and cauldron
			for (int j = 0; j < Players[activeplayer].Number_Graveyard; j++)
			{
				if (strcmp(Medium.BasicInfo.CardColour, Players[activeplayer].Card_Graveyard[j].CardColour) == 0)
				{
					Number_MediumColour++;
				}
			}
			for (int j = 0; j < Cauldrons[Medium.BattleCauldron - 1].Number_CardsInCauldron; j++)
			{
				if (strcmp(Cauldrons[Medium.BattleCauldron - 1].PileColour, Medium.BasicInfo.CardColour) == 0)
				{
					Number_MediumColour++;
				}
			}

			BattleCard Medium2;                                                       //current card to be tested, and will decide whether change the medium to medium2
			...
			if (strcmp(Medium2.BasicInfo.CardColour, "green") != 0)
			{
				int Number_Medium2Colour = 0;
				for (int j = 0; j < Players[activeplayer].Number_Graveyard; j++)           //count how many card of this colour in graveyard and cauldron 
				{
					if (strcmp(Medium2.BasicInfo.CardColour, Players[activeplayer].Card_Graveyard[j].CardColour) == 0)
					{
						Number_Medium2Colour++;
					}
				}
				for (int j = 0; j < Cauldrons[Medium2.BattleCauldron - 1].Number_CardsInCauldron; j++)
				{
					if (strcmp(Cauldrons[Medium2.BattleCauldron - 1].PileColour, Medium2.BasicInfo.CardColour) == 0)
					{
						Number_Medium2Colour++;
					}
				}
				if (Number_Medium2Colour > Number_MediumColour)                   // choose more card number to maximize this colour
				{
					Medium = Medium2;
		...
		else if (strcmp(Medium.BasicInfo.CardColour, "green") == 0)                   //if the card is green
		{
			BattleCard Medium2;                                                       //current card to be tested, and will decide whether change the medium to medium2
			Medium2.BasicInfo = Players[activeplayer].Card_Hand[i];
			Medium2.IndexInPlayerHand = i;
			Medium2.BattleCauldron = SearchRightCauldron(Medium2);
			if (strcmp(Medium2.BasicInfo.CardColour, "green") != 0)                 //if next is not green change into not green card
			{
				Medium = Medium2;
			}
			else                                                                    //the next is green, too
			{
				if (Medium.BasicInfo.CardValue < Medium2.BasicInfo.CardValue)       //choose the card with cigger value like drop the dangerous card
				{
					Medium = Medium2;
				}
			}
...
BattleCard ChooseOptimalCard()    //choose the smallest and smallest value of caudron
{
...
void EasyMoveAndCauldronExplosion(int activeplayer)
{
	...
	{
		BattleCard  mediumBC = ChooseMaxColourCard(activeplayer - 1);        //move biggest one and least card in the pile
		Index_Card_Move = mediumBC.IndexInPlayerHand;
		Index_Cauldron_WhereMoveTo = mediumBC.BattleCauldron;
	}
