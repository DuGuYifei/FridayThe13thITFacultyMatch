...
int MoveCardIntoCauldron(int activeplayer)
{
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "green") == 0 || strcmp(Players[activeplayer].Card_Hand[0].CardColour, "blue") == 0) //if the first card is blue or green
	{
		Cauldrons[0].CardsInCauldron[Cauldrons[0].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 1
		Cauldrons[0].Number_CardsInCauldron++;
		return 0;
	}
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "white") == 0)                                      //if the first card is white
	{
		Cauldrons[1].CardsInCauldron[Cauldrons[1].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 2
		Cauldrons[1].Number_CardsInCauldron++;
		return 1;
	}
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "red") == 0)                                        //if the first card is red
	{
		Cauldrons[2].CardsInCauldron[Cauldrons[2].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 3
		Cauldrons[2].Number_CardsInCauldron++;
		return 2;
	}
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "violet") == 0)                                     //if the first card is violet
	{
		Cauldrons[3].CardsInCauldron[Cauldrons[3].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 4
		Cauldrons[3].Number_CardsInCauldron++;
		return 3;
	}
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "yellow") == 0)                                     //if the first card is yellow
	{
		Cauldrons[4].CardsInCauldron[Cauldrons[4].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 5
		Cauldrons[4].Number_CardsInCauldron++;
		return 4;
	}
	if (strcmp(Players[activeplayer].Card_Hand[0].CardColour, "black") == 0)                                      //if the first card is black
	{
		Cauldrons[5].CardsInCauldron[Cauldrons[5].Number_CardsInCauldron] = Players[activeplayer].Card_Hand[0];   //move to pile 6
		Cauldrons[5].Number_CardsInCauldron++;
		return 5;
	}
	return -1;
}
...
void CauldronExplosion(int movetowhichcauldron, int activeplayer)
{
	int SumValueOfMoveCauldron = 0;                                                                   //add all values to check whether it should explode
	for (int i = 0; i < Cauldrons[movetowhichcauldron].Number_CardsInCauldron; i++)
	{
		SumValueOfMoveCauldron += Cauldrons[movetowhichcauldron].CardsInCauldron[i].CardValue;
	}
	if (SumValueOfMoveCauldron >= ExplodeThreshold)                                                    //if it is bigger than threshold
	{
		for (int i = 0; i < Cauldrons[movetowhichcauldron].Number_CardsInCauldron; i++)                //for every card in this pile
		{
			Players[activeplayer].Card_Graveyard[Players[activeplayer].Number_Graveyard] = Cauldrons[movetowhichcauldron].CardsInCauldron[i];   //copy into the graveyard
			Players[activeplayer].Number_Graveyard++;                               //increase the graveyard
			Cauldrons[movetowhichcauldron].CardsInCauldron[i].CardValue = 0;        //reinitialize the pile
			strcpy_s(Cauldrons[movetowhichcauldron].CardsInCauldron[i].CardColour, "0");
		}
		Cauldrons[movetowhichcauldron].Number_CardsInCauldron = 0;                   //reinitialize the pile
	}
}
...
{
	int MoveToWhichCauldron;
	MoveToWhichCauldron = MoveCardIntoCauldron(activeplayer - 1); //because the arry index need minus 1

	ResortPlayerHandCard(activeplayer - 1);

	CauldronExplosion(MoveToWhichCauldron, activeplayer - 1);
}
